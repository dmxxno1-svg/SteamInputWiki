...
=======
# 02c：混合输入（Mixed Input）

混合输入指同时使用多种输入方式（例如触控板 + 摇杆 + 陀螺），以发挥各自优势：触控板用于快速转向，陀螺用于精确瞄准，摇杆用于移动或大幅度视角调整。

设计要点：
- 明确主/辅输入的分工
- 避免输入冲突或重复映射
- 使用模式切换（Mode Shift）在需要时改变输入行为

当正确配置时，混合输入能够在不牺牲精度的前提下显著提升响应速度与操作灵活性。
>>>>>>> Stashed changes
